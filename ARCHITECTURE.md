...
7. **Telegram admission settlement concentration — shaped and approved under decision 0046.** `UpdateGate.runUpdate<T>` will execute update boundaries and consume an `AdmissionResult<T>` returned by the authoritative runtime/delegated-run admission or attachment operation: runtime/delegated-run handoff, structural rejection, or no-runtime completion. Telegram adapters do not manufacture decisions. The gate, rather than a caller-held release handle, owns finalization and coalesced claim groups; update completion remains independently drained so runtime disposal cannot wait on steering, one-shot delivery, or revival.
