...
- **ACP continuation**: Recovery after a clean detach: Goblin starts a fresh ACP server process, resumes the existing ACP session, reapplies policy, and sends a code-owned prompt to inspect current state and continue. It is not process adoption or exactly-once execution; same-boot hard-crash records without clean-detach proof are not auto-resumed.
