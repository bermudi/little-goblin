...
**Why:** `intake.handleText(message, text)` needs a `TelegramIntakeMessage` to resolve the session, build the `MessageBuffer`, and run `prepare`. Every fragment that lands in one buffer shares the same `(chatId, topicId, fromUserId)` key, which means they share `locator`, `isSupergroup`, `threadId`, and `reply`/`prepare` closures by construction (all derived from the same `ctx.chat`/`ctx.msg`). Carrying the first fragment's `message` is therefore equivalent to carrying any fragment's, but it's the simplest invariant — one write, one read — and avoids per-fragment message bookkeeping.
