...
7. **Telegram admission settlement concentration — shaped and approved under decision 0046.** `UpdateGate.runUpdate<T>` will execute update boundaries and consume an `AdmissionResult<T>` whose authority is partitioned: runtime/delegated-run admission or attachment operations authoritatively report handoff or structural rejection (`busy`, `fenced`, `rejected`); the gate authoritatively reports its own closure (`closed`); the local adapter authoritatively reports `completed` for parsing or other work it owns with no runtime operation (malformed updates, topic-description changes, unknown commands, and like no-runtime paths). Adapters may map structural results but do not invent handoff or rejection outcomes. The gate, rather than a caller-held release handle, owns finalization and coalesced claim groups; update completion remains independently drained so runtime disposal cannot wait on steering, one-shot delivery, or revival.

**WIP limit: one implementation phase in progress, one plainly described next.** Machine-held work authority and guest runtime admission are implemented and automated-test verified; no live Telegram smoke evidence was produced for these cycles. Telegram admission settlement concentration is the approved next carve; ACP external agents under decision 0044 follows with its accepted backend and persistence scope unchanged.
