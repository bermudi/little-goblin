...
Recovery SHALL be conversation continuation, not process adoption or exactly-once execution. Goblin SHALL NOT resend the original task, claim the interrupted prompt continued offline, or use `session/load` to replay history. A legacy native record, missing server, changed fingerprint, absent resume capability, or failed resume SHALL claim pending `interrupted`; an expired record SHALL claim pending `timed_out`. Each outcome SHALL become terminal only after the required local cleanup is confirmed, without guessing another session id. Legacy PTY records SHALL follow verified migration cleanup before any terminal transition.
