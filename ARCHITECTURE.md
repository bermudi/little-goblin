...
**Litespec WIP limit: one active labeled issue, with at most one plainly described candidate next.** The decision-0046 runtime-authority sequence, including Telegram admission settlement concentration, is implemented and automated-test verified; no live Telegram smoke evidence was produced for these cycles. No later phase is active merely because it appears in this historical delivery order. Telegram Settings is active as issue #59 (`litespec/telegram-settings`, target contract `specs/telegram-settings/spec.md`); no candidate next is named. ACP issue #58 is closed after PASS and merged into this branch (accepted host, delegated-run, continuation, shutdown, and legacy-removal contracts); #59's final model-launch integration rides those contracts rather than legacy seams.
