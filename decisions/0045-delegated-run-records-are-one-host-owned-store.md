...
Storage migration SHALL be offline migration step 5 (filesystem state version 5) under decision 0038: generic and named instance directories move wholesale into the new tree, each legacy `meta.json` is wrapped into the record format as one historical invocation, ambiguous or malformed records fail the migration loudly without selecting a winner, and symlinked named-agent roots are read through. Named-agent definitions remain user-authored under `workspace/agents/<name>/`; only machine-managed instances move. `scratch/external-agents/` SHALL NOT be migrated by this step; its legacy adapters are being replaced under decision 0044 and their records migrate with the ACP cycle.
...
Append-only invocation entries still mutate the record file; the validated atomic-rewrite discipline from `meta.ts` moves into the store rather than being relaxed. Attached named agents are now honestly interrupted by process death: continuation requires explicit revival, matching current user-facing behavior, but nothing silently resumes. `scratch/subagents` and `workspace/agents/*/instances` remain readable legacy input until step 5 runs, and `scratch/external-agents` remains a live legacy tree until the ACP cycle, so "no scratch" is not yet true. The architecture layout, repair map, and backlog views must be amended, and the deployment must run `bun run migrate` before the new code will poll.
