...
- A buffer-factory seam: the composition root (`bot.ts`) injects a `createMessageBuffer` factory when constructing the dispatcher, and Telegram intake supplies the factory implementation (Telegram rendering + the `onTopicNotFound` orphan-archive hook). The dispatcher uses the injected factory for both live and scheduled turns — it never constructs a `MessageBuffer` itself. The existing-but-half-wired injection hook becomes the only path; the internal `new MessageBuffer` fallback is removed. (Today the hook is used in tests and forwarded by `createTelegramIntake`, but prod `bot.ts` does not pass it and so the fallback fires. This change makes the factory mandatory.)
