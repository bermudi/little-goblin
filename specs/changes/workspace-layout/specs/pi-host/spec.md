...
The `src/pi-host.ts` module SHALL expose canonical path helpers for Goblin-owned prompt files under `$GOBLIN_HOME/workspace/`, including `agentsMdPath(home)` and `soulMdPath(home)`.
