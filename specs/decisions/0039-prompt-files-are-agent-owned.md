# Prompt Files Are Agent-Owned

## Status

accepted

Amends decision 0003 (main-goblin-prompt-ownership), which described `SOUL.md` as deployment-owned.
...
- **External-agent child processes** — decision 0012: same-user filesystem access beyond `projectDir` is a stated residual risk; adapter confinement is defense-in-depth, not a sandbox.
...
Therefore a reserved-path guard on `write`/`edit` could only ever prevent accidents. It could never be a security boundary, and claiming otherwise would contradict decision 0012's own honesty about the same-user trust boundary.
...
This is not a security boundary. Per decision 0012, operators requiring real isolation run Goblin under OS-level sandboxing.
...
Three canon statements now contradict this decision and require a patch: `specs/glossary.md:75` and `specs/canon/agent/spec.md:415` both call `SOUL.md` "deployment-owned", and decision 0003 needs a pointer to this record. `ARCHITECTURE.md`'s prompt-architecture section and its open-work item 1 need the same correction.
