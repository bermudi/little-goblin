...
- **`acp-external-agents`** — external ACP runner. The lifecycle substrate is complete; fresh work still requires current-code scouting and owner classification of the frozen proposed ACP boundary (legacy decision 0030).
- **`delegated-work-ownership`** — decision 0036 is accepted, but implementation remains deferred until the ACP boundary is classified and a fresh plan replaces the proposal's stale migration/reconciliation wording.
...
- v1.x: retroactive cleanup of orphaned subagents — subagents spawned before `cascade-cancel` may have been orphaned by a `/new`, `/resume`, `/archive`, or `/project` disposal; on startup or at a scheduled interval, find running `meta.json` files under `scratch/subagents/` and `workspace/agents/<name>/instances/` whose `spawnedBy` points to a session that no longer has a running runner, and cancel them.
...
- **Observability changes deleted during architecture stabilization (2026-07-26).** Four metrics changes and one external-agent durability change were deleted rather than archived, because `litespec archive` merges deltas into canon and would falsely claim the behavior is implemented. All artifacts remain in git history at commit `74d4769`; recover with `git show 74d4769:specs/changes/<name>/proposal.md` or `git checkout 74d4769 -- specs/changes/<name>`.
  ...
  - **`external-agent-metrics`** (27 tasks). Already self-marked *"Blocked for replan: `acp-external-agents` removes PTY fallback and generalizes backend ids."* PTY counters, adapter-kind fields, and per-agent summaries all need replanning. **Reopen after the external-agent protocol boundary is settled.**
