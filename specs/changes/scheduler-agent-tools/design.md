...
**Provenance re-stamping on `/schedule` mutation.** Any `/schedule`-path mutation of an existing record (`pause`, `resume`, `heartbeat on`, `heartbeat off`) SHALL re-stamp `source` to `"user"`. This "last writer owns" principle ensures that once the user touches a schedule, the agent cannot subsequently undo the user's action — e.g. the agent cannot re-enable a heartbeat the user disabled, or resume a schedule the user paused. The agent path never re-stamps (it can only touch agent-owned records, so re-stamping to `"agent"` would be a no-op). The only way for the agent to regain control is for the user to `remove` the schedule so the agent can create a fresh one. Re-stamping is implemented in `setState` (used by `pause`/`resume`) and `setHeartbeat`'s existing-record branch, gated on `agent === false`.
...
- **`src/scheduler/store.ts`** — the core change. (a) `create()` accepts optional `source`; stamp it on the record. (b) The `source` field defaults to `"user"` when absent in loaded records, enforced via `effectiveSource()` at every consumption site (cap counting, authority checks, list redaction, display) rather than by mutating records at load time — so legacy files are cap-safe and authority-protected without a normalization pass. (c) **Cap enforced at the mutation boundary**: `create`, `resume`, and `setHeartbeat(enabled:true)` SHALL refuse when the result would exceed `MAX_AGENT_SCHEDULES` enabled agent-source schedules for the session — implemented by a cap-aware code path distinct from the cap-exempt `/schedule` caller (e.g. an `enforceAgentCap` parameter or a dedicated agent-facing mutation method). (d) **Authority checks** on `remove`/`pause`/`resume`/`setHeartbeat` reject when the target record's effective `source` is `"user"` and the caller is the agent path. (e) **Provenance re-stamping**: any `/schedule`-path mutation of an existing record (`pause`, `resume`, `setHeartbeat` on/off) SHALL re-stamp `source` to `"user"`, so the user's actions claim ownership and the agent cannot subsequently undo them. Satisfies: *Schedule records carry provenance*, *Agent-originated schedules are bounded by a per-session cap*, *Agent tool authority is scoped to agent-owned schedules*.
