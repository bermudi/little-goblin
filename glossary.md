...
- **Execution Environment**: Immutable filesystem authority for a Conversation: either `personal` (the persistent `$GOBLIN_HOME/workspace`) or `project` (a validated canonical project root). It determines CWD, project guidance, environment skill roots, and project-bound tools; many Surfaces may share one environment.
...
- **named subagent**: A subagent that loads its `AGENTS.md` and isolated `.agents/skills/` catalog from `$GOBLIN_HOME/workspace/agents/<name>/`; it does not inherit the caller's resolved manifest by default.
...
- **work run**: One delegated subagent or external-agent task with an explicit attached or durable lifetime, immutable Execution Environment, origin Surface, execution outcome, and separate delivery state.
