...
**Pi execution host seam — complete.** `PiSubagentHost` owns Pi session construction and per-invocation execution leases; `SubagentRunner` and `execution.ts` own coordinator-side memory/tool assembly and durable lifecycle transitions. The remaining delegated-work ownership, cross-run lifetime, and durable-result work stays parked under `delegated-work-ownership`.
