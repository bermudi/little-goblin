...
The model MAY select an external agent's working directory, invocation parameters, and permission profile. The Devin model identity is excluded from model-facing selection: it is the operator-owned deployment default managed through Settings (decision 0049). Supported profiles SHALL include an unattended dangerous profile that permits the configured backend to act without interactive approval. An omitted profile selection defaults to the unattended dangerous profile. These selections remain structured external-agent inputs and SHALL be captured with the delegated-run record so the actual execution context is observable. This ruling does not decide whether backend executable selection or explicit child-environment overrides become model-facing.
...
External agents can work across repositories, choose backend options appropriate to the task, and run unattended under the dangerous default unless the model explicitly selects a narrower profile. The implementation must replace the current fixed project-directory tool input and two-profile schema with explicit model-facing launch choices, validate their structure, and record the selected context without presenting validation as confinement.
