...
Callers feed in raw policy knobs rather than asking "build context for this caller." `AgentRunner` passes `includeAgents: true`, `promptText` into `formatSnapshot` (`src/agent/mod.ts:375-381`) and `includeAgents: true` into `createMemorySearchTool` (`src/agent/mod.ts:193`). Subagent execution passes `includePersona`/`includeAgents` into `formatSnapshot` (`src/subagents/execution.ts:200-207`, a single call) and a `persona` policy into `createMemorySearchTool` (`src/subagents/execution.ts:143-146`). The interface is policy-knob-shaped, so the visibility rules for main agent vs named subagent vs anonymous subagent live implicitly across the memory layer and its call sites.
