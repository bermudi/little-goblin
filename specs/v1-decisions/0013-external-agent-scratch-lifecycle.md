...
External-agent run records SHALL persist under `$GOBLIN_HOME/scratch/external-agents/<runId>/`. `meta.json` and `result.txt` SHALL be written atomically; `events.jsonl` SHALL be appended as complete JSON lines. Each run is non-resumable: at startup, the runner SHALL load all persisted metadata, clean up PTY sessions belonging to the current owner, and atomically mark every non-terminal record `interrupted`, because no live handle can be proven owned after restart. Orphaned child processes from a prior run SHALL be terminated during this reconciliation: the runner SHALL use a verifiable process-group ownership mechanism (the persisted adapter kind and run ownership metadata) to safely terminate and reap owned PTY children before marking records interrupted, or block project reuse until that child exits. Persisted status SHALL NOT be reported as stopped while owned work remains active.

Output SHALL be bounded: 32,000 characters per normalized output event, 2 MiB per `events.jsonl` measured on UTF-8 bytes, 128,000 characters per final result, 16,000 characters per `status` response, and the 20 newest owned runs per `list` response. Truncation SHALL be explicit in metadata and tool results. The 2 MiB `events.jsonl` cap is measured on UTF-8 bytes; if the next serialized event would exceed the cap, the runner SHALL truncate the payload before serialization or reject the event, and SHALL never write a partial JSONL line or silently exceed the cap. Concurrent event writers for a single run SHALL be serialized through a per-run queue or mutex so that the UTF-8 size check, payload truncation or rejection, and complete-line append are performed atomically per event — concurrent writers cannot interleave JSON fragments or collectively exceed the 2 MiB cap.
