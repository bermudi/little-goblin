...
const TELEGRAM_OPS = ["sendMessage", "sendRichMessage", "sendMessageDraft", "sendRichMessageDraft", "editMessageText"] as const;
...
function isTelegramOp(value: unknown): value is TelegramOp {
  return value === "sendMessage" || value === "sendRichMessage" || value === "sendMessageDraft" || value === "sendRichMessageDraft" || value === "editMessageText";
}
...
function aggregateTelegramEvent(summary: TelegramMetricsSummary, event: Record<string, unknown>): void {
  ...
  if (op === "sendMessage" || op === "sendRichMessage" || op === "sendMessageDraft" || op === "sendRichMessageDraft") {
    summary.sendTotal++;
