...
- [ ] When this change is archived to canon: amend the archived `scheduled-turns` spec (or add a superseded-by annotation) so the canon `schedulesPath` contract reflects `state/schedules.json` instead of the root-level `schedules.json`. _(Conditional: `scheduled-turns` is still an active change, not yet archived — this amendment triggers at archive time, not now. Leaving unchecked until then.)_
