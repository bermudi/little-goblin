...
The machine is a **current generation** (`idle` / `preparing` / `active`) plus a **drain set** of prior generations whose disposal is still settling. A replacement creation may be reserved while a prior generation drains — the two coexist as distinct generations distinguished by a monotonic **runtime epoch**. Queue callers declare a closed work intent (`current-runtime`, `binding`, `internal-runtime`, or `bootstrap`); the machine synchronously creates, stores, checks, and settles the authority ticket and passes a machine-generated context to work for checks around awaits. Queue entries never store caller-provided predicates. Current-runtime and internal-runtime tickets are tied to one registered runner, so replacement fences them; active internal replacement moves the prior runner into the drain set before installing the new registration. Binding tickets preserve acknowledged command order through same-binding settings invalidation. Bootstrap tickets begin under binding/queue authority and must adopt the current registered runner before model or user-visible effects; cold scheduled turns use bootstrap while warm scheduled turns remain tied to their admission runtime. Binding change drops queued work.

The prompt queue is an explicit entry list driven by a serial executor. Its transport-neutral immediate/no-wait admission atomically checks occupancy and installs work only when idle, returning explicit accepted/busy/closed/fenced classifications. Immediate work never waits behind prompts or commands. Cold accepted work starts with bootstrap authority and adopts the registered runner before prompting; warm work remains tied to the admission runner. Ordinary cold text/media use the same bootstrap-before-preparation ordering. Runtime creation cancellation races every preparation await, observes any detached rejection, and prevents a cancelled candidate from committing. Every accepted immediate turn is fenced and drained through the same queue and shutdown path; after the final authority commit, one-shot delivery is initiated synchronously but its observed network promise is update-owned, so queue settlement and runtime disposal do not await Telegram. Cancelling queued work removes the entry and never touches the runner. Illegal machine transitions fail loud with structured identity. Seeded interleaving property tests assert that no work executes after its authority is fenced, no ticket leaks, and shutdown always terminates.
...
**CURRENT — decision 0046 runtime machine.** `ConversationRuntimeHost` is a thin coordinator over per-conversation `RuntimeMachine` instances. The machine owns the current generation (runner, creation reservation, queue, skill context), the drain set of prior generations, and the epoch counters. The host owns the process shutdown promise; the process-level admission gate is owned by `UpdateGate` (`src/shutdown/update-gate.ts`). The machine consolidated thirteen scattered collections into one coherent owner, but the host's public surface changed: `schedule` now takes a closed `WorkIntent` (the machine derives and holds the epoch ticket synchronously at enqueue) instead of a caller-supplied `isCurrent` callback, and `captureEpoch`/`isEpochCurrent`, `admitImmediateRuntimeWork`, `steerOrQueue`, and the admission accessors (`isAdmissionOpen`/`closeAdmission`/`assertAdmissionOpen`) are new public members. `schedulePrompt` and `scheduleCommand` are dispatcher methods that only declare intent (`current-runtime`/`internal-runtime` vs `binding`); the runtime-vs-binding epoch is captured inside `RuntimeMachine.schedule` from that intent, not by the dispatcher. Commit-point authority is not a single helper: non-queue creation/attachment points compare `isEpochCurrent(axis, epoch)` (e.g. the `PreparedRuntimeAssembler` checkpoint and the revived-subagent check), the queue serial executor compares `isQueueAuthorityCurrent` via the `WorkAuthority.isCurrent()` it hands each `run`, and runner registration uses `isRegisteredRunner` / `isCurrentCreation`. Steer-vs-queue is decided synchronously from `runner.isStreaming` — the bounded observation window is deleted. Scheduler claim and enqueue are atomic: a rejection structurally restores the claim.

**CURRENT — shutdown coordinator and Telegram settlement.** `ShutdownCoordinator` (`src/shutdown/coordinator.ts`) owns one ordered phase list (`SHUTDOWN_PHASE_NAMES`): close the Telegram gate, let buffered text reach runtime admission, start runtime disposal before awaiting the Telegram drains, stop polling, then drain subsystems in order. The causal ordering that lived as comments in `index.ts` is now data. `UpdateGate` (`src/shutdown/update-gate.ts`) owns process-level authorization, structural admission settlement, completion tracking, and coalescer close coupling. `runUpdate<T>` consumes one typed `AdmissionResult<T>` instead of exposing release capabilities: runtime/delegated operations own `handoff` and structural rejection, adapters own no-runtime `completed`, and only the gate owns `closed`. Structural decisions release the runtime-admission drain before their completion, while coalesced fragment claims receive one merged result atomically. `index.ts`'s shutdown body is one coordinator call. Runtime admission also fences any scheduler dispatch that was already in flight.
...
Pi context-file auto-discovery stays disabled for the main runtime. Project guidance is supplemental and must not replace deployment identity. **CURRENT.** Prompt-file reads go through `WorkspacePrompts` (`src/workspace/prompts.ts`), the sole prompt-file reader: it owns the deployment prompt-file catalog and applies required/optional semantics (required ENOENT throws `MissingSoulError`; optional ENOENT yields absent; non-ENOENT propagates). Path construction stays in the path-helper modules (decisions 0008, 0009 as amended by 0050).
...
The implemented machine-managed filesystem sequence is Surface identity (step 1), immutable Execution Environments (step 2), transcript Surface provenance (step 3), Conversation lifecycle ownership (step 4), delegated-work layout break (step 5), then inner-life wake layout (step 6). The current filesystem gate is state version 6. Step 5 creates `state/delegated-work/runs/` and advances the version; it does not transform legacy subagent trees. Step 6 creates `state/inner-life/wakes/`, applies the memory SQLite schema migration that creates the effect-receipt table, and converts legacy light-sleep cursor locations (`memory-reflection.json` files and `dreaming_cursor:<id>` memory_meta rows) into the sidecar cursors the host reads, leaving existing memory rows untouched. Native skill layout was an operator-owned move and does not participate in this sequence.
...
4. **Runtime authority consolidation — implemented under decision 0046.** One per-conversation `RuntimeMachine` behind the `ConversationRuntimeHost` port owns admission, generation identity, queueing, and disposal. Authority is held as epochs (runtime epoch, binding epoch) and compared at commit points rather than re-derived at module boundaries; the prompt queue is an explicit entry list with a serial executor; `ShutdownCoordinator` walks one owned phase list; `UpdateGate` owns process-level admission and typed settlement. Steer-vs-queue is decided synchronously; the bounded observation window is deleted. Scheduler claim and enqueue are atomic. The superseded single-check guardrail clause is replaced by the commit-point discipline (decision 0046).
5. **Machine-held work authority — implemented under decision 0046.** Queue callers now declare a closed intent instead of supplying epoch predicates. `RuntimeMachine` owns each ticket and its generated await-check context. Current runtime turns, binding commands, internal turns, steer fallback, and warm/cold scheduled turns use the appropriate machine authority; cold scheduled work adopts the registered runner before effects, and internal replacement fences prior tickets while retaining the prior runner as a tracked draining generation.
6. **Runtime-authorized guest turns — implemented under decision 0046.** One machine-owned immediate/no-wait path atomically classifies occupancy and installs accepted work before asynchronous runner acquisition. Guest cold/warm authority, prompt execution, stale success/error suppression, runtime settlement, and shutdown drain live behind `TurnDispatcher`; Telegram intake retains Surface resolution, opaque one-shot delivery, article mapping, and `GuestReplySink` only. Delivery is initiated after the final machine-authority check, then awaited by the released Telegram handler rather than the runtime queue.
7. **Telegram admission settlement concentration — implemented under decision 0046.** `UpdateGate.runUpdate<T>` executes update boundaries and consumes an `AdmissionResult<T>` whose authority is partitioned: runtime/delegated-run admission or attachment operations authoritatively report handoff or structural rejection (`busy`, `fenced`, `rejected`); the gate authoritatively reports its own closure (`closed`); the local adapter authoritatively reports `completed` for work it owns with no runtime operation. The gate, rather than a caller-held release handle, owns finalization; update completion remains independently drained so runtime disposal does not wait on steering, one-shot delivery, or revival. Coalesced text stores opaque fragment claims and settles each merged group atomically. Pre-decision throws and missing or contradictory decisions fail loud. Runtime late-steer fallback uses a typed `RunnerNotStreamingError`, not rejection string parsing.

**Litespec WIP limit: one active labeled issue, with at most one plainly described candidate next.** The decision-0046 runtime-authority sequence, including Telegram admission settlement concentration, is implemented and automated-test verified; no live Telegram smoke evidence was produced for these cycles. No later phase is active merely because it appears in this historical delivery order. Telegram Settings issue #59 is closed after PASS (`litespec/telegram-settings`, target contract `specs/telegram-settings/spec.md`); no active labeled issue and no candidate next is named. ACP issue #58 is closed after PASS and merged into this branch (accepted host, delegated-run, continuation, shutdown, and legacy-removal contracts); #59's final model-launch integration rides those contracts rather than legacy seams.
