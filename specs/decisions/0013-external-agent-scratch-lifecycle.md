...
Output SHALL be bounded: 32,000 characters per normalized output event, 2 MiB per `events.jsonl`, 128,000 characters per final result, 16,000 characters per `status` response, and the 20 newest owned runs per `list` response. Truncation SHALL be explicit in metadata and tool results.
