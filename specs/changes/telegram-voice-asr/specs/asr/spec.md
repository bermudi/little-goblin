...
The system SHALL provide an internal ASR module with a narrow provider-shaped function for transcribing audio bytes. The only implemented provider SHALL be Groq. The Groq implementation SHALL send a multipart request to `https://api.groq.com/openai/v1/audio/transcriptions` with the configured model and Groq API key (transmitted as an HTTP `Authorization: Bearer` token), parse the returned transcript text, and return a typed result without throwing for ordinary API failures.
...
- `{ ok: false; error: string }` — failure. `error` is a sanitized, non-secret message. Failures include non-2xx responses, malformed JSON, missing key (when a caller passes an empty/undefined API key), and network/timeout failures.

The ASR module MUST NOT include the Groq API key (bearer token) in any result or log line.
...
- **WHEN** the Groq request exceeds the hardcoded 30-second timeout
